...
Octree* Octree::getRegionFromPoint(double x, double y, double z) {
	bool isChild = true;

		for (int i = 0; i < 8; i++) {
			if (children[i] != NULL) {
				isChild = false;
				if (x < children[i]->minX ||
					x > children[i]->maxX ||
					y < children[i]->minY ||
					y > children[i]->maxY ||
					z < children[i]->minZ ||
					...
					// Outside child's box, so skip
				}
				else {
					Octree *child = children[i]->getRegionFromPoint(x, y, z);
					if (child != NULL) {
						return child;
					}
				}
